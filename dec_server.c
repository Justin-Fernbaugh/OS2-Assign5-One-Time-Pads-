...
/*-------------------------------------------------*/
/*
	Iterates through plaintext decrypting each char with 
	the same index in the key+originalIndex
*/
void decryptMsg(char* plaintext, char* encryptKey, char* cipherText) {
	for(int i=0; i < strlen(plaintext); i++) {
		if(plaintext[i] == 32) {
			strncat(cipherText, " ", 1);
		} else {
			// converting in range 0-25
			char x = (plaintext[i] - encryptKey[i] + 26) %26;
...
void getEncryptParts(int socketConn, struct sockaddr_in clientAddress) {
	...
	decryptMsg(plaintext, encryptKey, encryptedMessage);
